#ident "@(#)$Id: table.c,v 1.1 1996/08/14 18:42:32 anton Exp $";

#include "b.h"

static void EXFUN(growIndex_Map, (Index_Map *));
static Relevant EXFUN(newRelevant, (NOARGS));
static Dimension EXFUN(newDimension, (Operator, int));
static void EXFUN(GT_1, (Table));
static void EXFUN(GT_2_0, (Table));
static void EXFUN(GT_2_1, (Table));
static void EXFUN(growTransition, (Table, int));
static Item_Set EXFUN(restrict, (Dimension, Item_Set));
static void EXFUN(addHP_1, (Table, Item_Set));
static void EXFUN(addHP_2_0, (Table, Item_Set));
static void EXFUN(addHP_2_1, (Table, Item_Set));
static void EXFUN(addHyperPlane, (Table, int, Item_Set));

static void DEFUN(growIndex_Map, (r), Index_Map *r)
{
    Index_Map new;
...
static Relevant DEFUN_VOID(newRelevant)
{
    Relevant r = (Relevant) zalloc(max_nonterminal * sizeof(*r));
    return r;
}

void DEFUN(addRelevant, (r, nt), Relevant r AND NonTerminalNum nt)
{
    int i;
...
static Dimension DEFUN(newDimension, (op, dim), Operator op AND ArityNum dim)
{
    Dimension d;
    List pl;
    Relevant r;
	...
	Rule pr = (Rule) pl->x;
	if (pr->pat->op == op)
	    addRelevant(r, pr->pat->children[dim]->num);
    }
...
Table DEFUN(newTable, (op), Operator op)
{
    Table t;
    int i, size;
...
static void DEFUN(GT_1, (t), Table t)
{
    Item_Set	*ts;
...
static void DEFUN(GT_2_0, (t), Table t)
{
    Item_Set	*ts;
    ItemSetNum 	oldsize = t->dimen[0]->max_size;
    ItemSetNum 	newsize = t->dimen[0]->max_size + TABLE_INCR;
...
static void DEFUN(GT_2_1, (t), Table t)
{
    Item_Set	*ts;
...
static void DEFUN(growTransition, (t, dim), Table t AND ArityNum dim)
{
    assert(t);
    assert(t->op);
...
static Item_Set DEFUN(restrict, (d, ts), Dimension d AND Item_Set ts)
{
    DeltaCost	base;
    Item_Set	r;
    int found;
...
static void DEFUN(addHP_1, (t, ts), Table t AND Item_Set ts)
{
    List pl;
    Item_Set e;
    Item_Set tmp;
...
static void DEFUN(addHP_2_0, (t, ts), Table t AND Item_Set ts)
{
    List pl;
    register Item_Set e;
    Item_Set tmp;
...
static void DEFUN(addHP_2_1, (t, ts), Table t AND Item_Set ts)
{
    List pl;
    register Item_Set e;
    Item_Set tmp;
...
static void DEFUN(addHyperPlane, (t, i, ts), Table t AND ArityNum i AND Item_Set ts)
{
    switch (t->op->arity) {
	default:
	    assert(FALSE);
...
void DEFUN(addToTable, (t, ts), Table t AND Item_Set ts)
{
    ArityNum i;
...
Item_Set *DEFUN(transLval, (t, row, col), Table t AND int row AND int col)
{
    switch (t->op->arity) {
	case 0:
	    assert(row == 0);
...
void DEFUN(dumpRelevant, (r), Relevant r)
{
    for (; *r; r++)
	printf("%4d", *r);
}

void DEFUN(dumpIndex_Map, (r), Index_Map *r)
{
    int i;
...
void DEFUN(dumpDimension, (d), Dimension d)
{
...
void DEFUN(dumpTable, (t, full), Table t AND int full)
{
    int i;
...
void DEFUN(dumpTransition, (t), Table t)
{
    int i, j;
