#ident "@(#)$Id: item.c,v 1.1 1996/08/14 18:42:27 anton Exp $";

#include "b.h"

static Item_Set fptr;

ItemArray DEFUN_VOID(newItemArray)
{
    ItemArray ia;
...
ItemArray DEFUN(itemArrayCopy, (src), ItemArray src)
{
    ItemArray dst;
...
Item_Set DEFUN(newItem_Set, (relevant), Relevant relevant)
{
    Item_Set ts;
...
void DEFUN(freeItem_Set, (ts), Item_Set ts)
{
    assert(!fptr);
    fptr = ts;
}

int DEFUN(equivSet, (a, b), Item_Set a AND Item_Set b)
{
    register Relevant r;
    register int nt;
...
void DEFUN(printRepresentative, (f, s), FILE *f AND Item_Set s)
{
    if (!s) {
	return;
    }
...
void DEFUN(dumpItem, (t), Item *t)
{
    printf("[%s #%d]", t->rule->lhs->name, t->rule->num);
    dumpCost(t->delta);
}

void DEFUN(dumpItem_Set, (ts), Item_Set ts)
{
    int i;
...
void DEFUN(dumpCost, (dc), DeltaCost dc)
{
    printf("(%ld)", (long) dc);
}
