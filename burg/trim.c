#ident "@(#)$Id: trim.c,v 1.1 1996/08/14 18:42:33 anton Exp $";
...
static void EXFUN(siblings, (int i, int j));
static void EXFUN(findAllNexts, (NOARGS));
static Relation *EXFUN(newAllPairs, (NOARGS));

static void DEFUN(siblings, (i, j), int i AND int j)
{
    int k;
    List pl;
    DeltaCost Max;
...
static void DEFUN_VOID(findAllNexts)
{
    int i, j;
    int last;
...
static Relation *DEFUN_VOID(newAllPairs)
{
    int i;
    Relation *rv;
